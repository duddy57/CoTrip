<script lang="ts">
	...
	let {
		...
		data: { deleteTrip: SuperValidated<Infer<DeleteTripRequestTypeSchema>> };
		id: string;
	} = $props();

	const form = superForm(data.deleteTrip, {
		validators: zodClient(deleteTripRequestSchema),
		onSubmit() {
			isLoading = true;
		},
		onResult(result) {
	...
	// Reset phrase when dialog closes
...
<AlertDialog.Root bind:open={dialogOpen} onOpenChange={handleDialogChange}>
	...
	<AlertDialog.Content class="sm:max-w-lg">
		<AlertDialog.Header class="space-y-4">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<AlertDialog.Description class="text-red-700 dark:text-red-300">
						Você tem certeza que deseja excluir essa viagem?
			...
			<Alert.Root class="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/50">
				...
				<Alert.Description class="text-red-700 dark:text-red-300">
					Esta ação não pode ser desfeita. Todos os dados da viagem, incluindo membros, itinerários
					e despesas serão permanentemente removidos.
				</Alert.Description>
			</Alert.Root>
		</AlertDialog.Header>
		...
		<div class="space-y-4 py-4">
			...
			<form id="delete-trip-form" method="POST" action="?/deleteTrip" use:enhance>
